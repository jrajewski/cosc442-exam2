...
public class ListSavesCommand extends ServerCommand {
    ...
    public void run(int connId, String[] args) {
        ...
        //Bug: Possible null pointer dereference in megamek.server.commands.ListSavesCommand.run(int, String[]) due to return value of called method
        //Joseph Rajewksi
        if(saveGames != null){
	        for (int i = 0; i < saveGames.length; i++) {
	            updateServerSave(saveGames, i);
	        }
        }        
    }

	private void updateServerSave(File[] saveGames, int i) {
		if (saveGames[i].isFile()) {
		    checkSave(saveGames, i);
		}
	}

	private void checkSave(File[] saveGames, int i) {
